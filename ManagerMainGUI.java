...
public class ManagerMainGUI extends PortfolioFrame {
    ...
    private JButton createButton(String text, String imagePath) {
        ...
        JButton button = new JButton(text,
                new ImageIcon(new ImageIcon(imagePath).getImage().getScaledInstance(30, 30, Image.SCALE_SMOOTH)));
    ...
    private void handleSendNotificationButtonClick() {
        ...
        for (Customer c : Database.getCustomers()) {
            System.out.printf("%nUsername: %s | NetGain: %.2f | Status: %s | Statuscheck: %b", c.getUsername(),
                    c.getNetGain(), Database.getAccountStatus(c.getUsername()),
                    Database.getAccountStatus(c.getUsername()).trim().equalsIgnoreCase(Constant.CUSTOMER_STATUS));
            if ( // if customer meets super customer realised profit threshold and is not already
                 // a super user
            (c.getNetGain() >= Constant.SUPER_CUSTOMER_PROFIT_THRESHOLD) &&
                ...
                popup_notification_message += String.format("Username: %s with Realised Profit: %.2f  ",
                        c.getUsername(), c.getNetGain());
            }
        }
